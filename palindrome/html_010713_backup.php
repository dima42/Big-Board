...
<table width='1200'>
<tr><td><span style='font-size:18px;font-weight:bold;'>I Prefer Pi Big Board</span></td>
_HTML_;
if ($showTeamInformation) {
	print <<<_HTML_
<td align='right'><strong>HQ</strong>: 4-149 (main), 4-146, 4-256&nbsp;&nbsp;&nbsp;<strong>Phone</strong> 617-715-4577&nbsp;&nbsp;&nbsp;<strong>E-mail</strong>: team-palindrome@mit.edu</td>
_HTML_;
}
...
function writeIntro() {
	...
	print "<p>News: <input id='UrgentMessage' name='UrgentMessage' value='".$latest_news."' style='border: none; "
    		."background: transparent;' size=200 onchange='add_update(this, \"URG\", ".$_SESSION["user_id"].")'/><br /><span class='pastNews'><a href='?updates'>past news</a></span></p>";
    } else {
    print "<p>News: <input id='UrgentMessage' name='UrgentMessage' value='Type over this text to send out an urgent message.' style='border: none; "
    		."background: transparent;' size=200 onchange='add_update(this, \"URG\", ".$_SESSION["user_id"].")'/><br /><span class='pastNews'><a href='?updates'>past news</a></span></p>";
    }
}
...
<div id='new_puzzle_save' style='text-align: center; padding: 4px 2px 0px 2px;'>
...
function displayPuzzles($my_puzzle_list) {
	...
	while ($row = mysql_fetch_assoc($result)) {
		...
		if ($row["META"] == 1) {
			...
			print "<tr><th align=center valign=top name='puzzle_".$row['PUZID']."' style='border:1px solid #666666' class='MetaRound ".$row['STATUS']."' width=".$cell_width."px>".$puzzle_link."<br/>\r\n".
					$answer_field.
					"<a href='?meta=".$row['PUZID']."' class='fake_button'>info</a>&nbsp;".$sprdlink."&nbsp;".$is_puz_out."&nbsp;<img src='puzzle.png' border=1px height=14px width=14px/ onclick='show_puzzle_input(\"P\",".$row['PUZID'].", ".$_SESSION['user_id'].");'></th>\r\n";
		} else {
